...
const isNode = obj => {
    ...
    while (obj && 'constructor' in obj) {
        if (obj.constructor.name === 'Node') return true;
...
const traverser = (node, exitVisitor, indent = 0) => {
    ...
    Object.keys(node).forEach(key => {
        ...
        if (Array.isArray(node[key])) res[key] = node[key].map(v => traverser(v, exitVisitor, indent + 2))
        else                          res[key] = traverser(node[key], exitVisitor, indent + 2);
    });
...
const exitVisitor = {
    ...
    BinaryExpression: (node, res, indent) => {
        ...
        switch (node.operator) {
            ...
            case '/': return left / right;
            case '%': return left % right;
            default: throw new Error('対応していない演算子');
        }
    },
